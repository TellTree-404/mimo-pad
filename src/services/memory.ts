...
export async function addMemory(
  ...
  if (allEntries.length > MAX_MEMORY_ENTRIES) {
    ...
    for (const e of toDelete) {
      await storage.deleteMemoryEntry(e.id);
    }
  }
}
...
export async function buildMemoryContext(currentConversationId?: string): Promise<string> {
  const entries = await storage.getAllMemoryEntries();
  ...
  const items = sorted.map(
    (e) => `- [${new Date(e.timestamp).toLocaleString()}] ${e.content}`
  );

  return `\n\n## 历史重要信息 (Memory)\n以下是从之前对话中提取的重要信息，你可以在回答时参考：\n${items.join('\n')}\n`;
}
...
export async function extractKeyInsights(
  ...
  for (const topic of keyTopics) {
    if (topic.length > 5) {
      await addMemory(topic, 1, conversationId, ['auto-extracted']);
    }
  }
}
...
export function manageContextWindow(
  ...
  if (memoryStart && memoryStart < result[0]?.timestamp) {
  }
